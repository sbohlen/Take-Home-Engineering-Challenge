...
DATEPART(HOUR, td.lpep_pickup_datetime) as tripHour,

--calculate the AVG for all trips having the same hour
(select AVG(tdcosts.total_amount)
   	from green_tripdata tdcosts
	where tdcosts.total_amount > 0
    and DATEPART(HOUR, tdcosts.lpep_pickup_datetime) = DATEPART(HOUR, td.lpep_pickup_datetime)
...
--calculate the AVG for all trips having the same hour
(select top 1 AVG(DATEDIFF(minute, td.lpep_pickup_datetime, td.lpep_dropoff_datetime)) as answer
   	from green_tripdata tdtimes
	where
	DATEPART(HOUR, tdtimes.lpep_pickup_datetime) = DATEPART(HOUR, td.lpep_pickup_datetime)
...
--calculate the MIN for all trips having the same hour
(select top 1 MIN(DATEDIFF(minute, td.lpep_pickup_datetime, td.lpep_dropoff_datetime)) as answer
   	from green_tripdata tdtimes
	where
	DATEPART(HOUR, tdtimes.lpep_pickup_datetime) = DATEPART(HOUR, td.lpep_pickup_datetime)
	and tdtimes.PUBoroughID = td.PUBoroughID
...
--calculate the MAX for all trips having the same hour
(select top 1 MAX(DATEDIFF(minute, td.lpep_pickup_datetime, td.lpep_dropoff_datetime)) as answer
   	from green_tripdata tdtimes
	where
	DATEPART(HOUR, tdtimes.lpep_pickup_datetime) = DATEPART(HOUR, td.lpep_pickup_datetime)
	and tdtimes.PUBoroughID = td.PUBoroughID
...
group by DATEPART(HOUR, td.lpep_pickup_datetime), td.PUBoroughID, td.DOBoroughID
